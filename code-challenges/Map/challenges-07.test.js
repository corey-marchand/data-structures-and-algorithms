...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let yeeet = arr.map(value => Math.pow(2, value));
  return yeeet;
};
...
const charCode = (arr) => {
  // Solution code here...
  let charCood = arr.map(value => value.charCodeAt());
  return charCood;
};
...
const extractStats = (arr) => {
  // Solution code here...
  let awhoooooo = arr.map(value => {
    let sum = value.effort + value.baseStat;
    return {name: value.stat.name, total: sum};
  })
  return awhoooooo;
};
