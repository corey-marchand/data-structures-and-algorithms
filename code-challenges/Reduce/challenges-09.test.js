...
const reversedString = (str) => str.split('').reduce((acc, cur) => acc = cur + acc, '');
// Solution code here...
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let newArr = arr.reduce((answersSoFar, value) => {
    if(value.children){
      answersSoFar += value.children.length;
    }
    return answersSoFar;
  }, 0)
  return newArr;
...
const calculateAverage = (arr) => {
  // Solution code here...
  let newArr = arr.reduce((accumalator, currentVal) => {
    accumalator.count++;
    accumalator.sum += currentVal;
    return accumalator;
  }, { count: 0, sum: 0});
  let avg = newArr.sum / newArr.count;
  return avg;
};
