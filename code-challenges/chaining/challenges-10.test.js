...
const count = (target, input) => {
  // Solution code here...
  let newCount = input.map(arr => arr.reduce((acc, cur) => {
    if(cur === target){
      acc++;
    }
    return acc;
  }, 0));
  return newCount.reduce((cur, acc) => acc += cur, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let divByFive = input.map(arr => arr.filter(arr => arr % 5 === 0 && Number.isInteger(arr)));
  let pow = divByFive.map(arr => arr.reduce((acc, cur) => {
    acc.push(Math.pow(2, cur));
    return acc;
  }, []))
  return pow;
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.reduce((a, b) => (parseInt(a.height) < parseInt(b.height)) ? a : b).name;
  return shortest;
};
